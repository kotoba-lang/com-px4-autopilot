(ns px4_autopilot.main
  "Kotodama WASM entrypoint for the px4_autopilot clean-room actor (L4) — Clojure port.
  ...
  facts (`px4_autopilot.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "px4_autopilot")
(def tier "L4")
...
(def entity-specs
  [{:entity "Heartbeat"          :plural "heartbeats"          :id-prefix "px4autop_hea"
    :fields [:type :autopilot :baseMode :customMode :systemStatus :mavlinkVersion] :required [:type :autopilot]
    :coerce {}                    :refs {}}
   {:entity "SysStatus"           :plural "sysstatuses"         :id-prefix "px4autop_sys"
    :fields [:load :voltageBattery :currentBattery :batteryRemaining :dropRateComm :errorsComm] :required [:load :voltageBattery]
    :coerce {}                    :refs {}}
   {:entity "GpsRawInt"           :plural "gpsrawints"          :id-prefix "px4autop_gps"
    :fields [:timeUsec :fixType :lat :lon :alt :vel :satellitesVisible] :required [:timeUsec :fixType]
    :coerce {}                    :refs {}}
   {:entity "Attitude"            :plural "attitudes"           :id-prefix "px4autop_att"
    :fields [:timeBootMs :roll :pitch :yaw :rollspeed :pitchspeed :yawspeed] :required [:timeBootMs :roll]
    :coerce {:roll :float :pitch :float :yaw :float :rollspeed :float :pitchspeed :float :yawspeed :float} :refs {}}
   {:entity "GlobalPositionInt"   :plural "globalpositionints"  :id-prefix "px4autop_glo"
    :fields [:timeBootMs :lat :lon :alt :relativeAlt :vx :vy :vz :hdg] :required [:timeBootMs :lat]
    :coerce {}                    :refs {}}
   {:entity "MissionItem"         :plural "missionitems"        :id-prefix "px4autop_mis"
    ...
    :coerce {:x :float :y :float :z :float} :refs {}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"px4_autopilot.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "px4_autopilot-compat" :tier tier :entities entities} 200])
...
(defn start! [] :px4_autopilot-compat/ready)
