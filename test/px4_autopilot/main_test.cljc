(ns px4_autopilot.main-test
  "Contract + behavioral test for the px4_autopilot-compat L4 actor (cljc port).
...
(deftest healthz
  (let [[body status] (m/healthz)]
    (is (= 200 status)) (is (= "px4_autopilot-compat" (:actor body))) (is (= (set m/entities) (set (:entities body))))))

#?(:clj (defn -main [& _]
          (let [{:keys [fail error]} (run-tests 'px4_autopilot.main-test)]
